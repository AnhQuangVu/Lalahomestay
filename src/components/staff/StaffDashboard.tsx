  ...
  const loadRooms = useCallback(async () => {
    try {
      ...
      const mapped: Room[] = activeRooms.map((r: any) => {
        ...
        if (currentBooking) {
             if (currentBooking.trang_thai === 'cho_coc') {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.gridRooms}>
          ...
          const isOccupied = ['occupied', 'checkout-soon', 'pending'].includes(room.status); 

          return (
              <div 
                key={room.id} 
                onClick={() => { setSelectedRoom(room); setIsEditingInfo(false); }} 
                style={{ ...styles.roomCard, borderColor: room.status === 'pending' ? '#fcd34d' : '#f3f4f6' }}
